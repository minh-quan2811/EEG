...
def plot_ratio_analysis(results, fatigue_levels, valid_subjects):
    ...
    fig.suptitle("EEG Fatigue Analysis — (theta+alpha)/beta Ratio",
                 fontsize=16, fontweight="bold", y=0.98)
    gs = gridspec.GridSpec(2, 2, figure=fig, hspace=0.45, wspace=0.35)

    # --- Panel 1: Before vs After per subject ---
    ax1 = fig.add_subplot(gs[0, 0])
    x = np.arange(len(valid_subjects))
    w = 0.35
    b_vals = [np.nanmean([r for sess in results[s]["before"].values() for r in sess])
              for s in valid_subjects]
    a_vals = [np.nanmean([r for sess in results[s]["after"].values() for r in sess])
              for s in valid_subjects]
    ...
    ax1.set_ylabel("Mean (theta+alpha)/beta Ratio")
    ax1.set_title("Mean Ratio: Before vs After (per subject)")
    ax1.legend(fontsize=9)
    ax1.grid(axis="y", alpha=0.3)

    # --- Panel 2: Session trend ---
    ...
    for i, subject in enumerate(valid_subjects):
        ...
        ax4.scatter([i - 0.15] * len(before_zs), before_zs,
                    color="#3498db", alpha=0.6, s=40,
                    label="Before" if i == 0 else "")
        ax4.scatter([i + 0.15] * len(after_zs), after_zs,
                    color="#e74c3c", alpha=0.6, s=40,
                    label="After" if i == 0 else "")

    # draw a threshold line for each Z cutoff
    from ..config import Z_THRESHOLDS
    line_colors = ["#f1c40f", "#e67e22", "#e74c3c", "#8e44ad", "#2980b9"]
    for idx, thresh in enumerate(Z_THRESHOLDS):
        color = line_colors[idx % len(line_colors)]
        ax4.axhline(thresh, color=color, linestyle="--",
                    linewidth=1.2, label=f"Z={thresh:+.3f}")
