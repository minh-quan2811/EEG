...
def run_wilcoxon_analysis(results: dict, valid_subjects: list[str], config) -> tuple:
    print("\n" + "=" * 55)
    ...
    w_dict = {"N": len(before_means), "status": "insufficient"}
    if len(before_means) >= 3:
        ...
        eff = effect_size_interpretation(r_effect)
        ...
        print(f"  Effect size r     : {r_effect:.4f}  ({eff})")
    else:
        print(f"  [SKIP] Not enough subjects (need >= 3, have {w_dict['N']}).")
    return None, w_dict


def run_kruskal_wallis_analysis(results: dict, valid_subjects: list[str], config) -> tuple:
    print("\n" + "=" * 60)
    ...
    if len(groups) >= 2:
        ...
        print("\n  Post-hoc Dunn's (Bonferroni corrected):")
        for a, b in all_pairs:
            g1, g2 = level_ratios.get(a, []), level_ratios.get(b, [])
            if len(g1) < 2 or len(g2) < 2:
                print(f"    Level {a} vs {b}: insufficient data")
                continue
            u_stat, p_raw = stats.mannwhitneyu(g1, g2, alternative='two-sided')
            p_corrected = min(p_raw * n_pairs, 1.0)
            sig2 = "[SIGNIFICANT]" if p_corrected < 0.05 else "[NOT SIGNIFICANT]"
            la, lb = config.LEVEL_LABELS[a], config.LEVEL_LABELS[b]
            print(f"    {la} vs {lb}: U={u_stat:.1f}, p(corr)={p_corrected:.4f}  {sig2}")
    else:
        print("  [SKIP] Not enough non-empty groups.")
    return h_stat, p_kw, level_ratios


def run_chi_square_analysis(fatigue_levels: dict, valid_subjects: list[str], config) -> tuple:
    print("\n" + "=" * 60)
    ...
    chi2_result = (chi2, p_chi, dof, cramers_v)
