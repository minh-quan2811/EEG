...
def compute_band_power_per_window(filepath, bands, window_sec=4.0, overlap=0.5):
    """Load .set file and return per-window band powers. List of dicts, one per window."""
    try:
...
def compute_band_power(filepath, bands, nperseg_sec=4):
    """Load EEG file and return band power dict aggregated across channels and windows."""
    try:
        raw = mne.io.read_raw_eeglab(filepath, preload=True, verbose=False)
        sfreq = raw.info['sfreq']
        data = raw.get_data() * 1e6
        ...
        for band_name, (fmin, fmax) in bands.items():
            ...
            for ch in range(data.shape[0]):
                ...
                idx = np.logical_and(freqs >= fmin, freqs <= fmax)
                band_powers.append(_trapz(psd[idx], freqs[idx]))

            power[band_name] = float(np.mean(band_powers))
